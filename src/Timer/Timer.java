...
public abstract class Timer {
    ...
    /**
     * Default constructor which sets the interval to 1s and the duration to
     * {@link Timer#DURATION_INFINITY}
     */
    public Timer() {
    ...
    /**
     * @param interval The time gap between each tick in seconds.
     * @param duration The period in seconds for which the timer should run. Set
     * it to {@code Timer#DURATION_INFINITY} if the timer has to run
     * indefinitely.
     */
        ...
        future = execService.scheduleWithFixedDelay(new Runnable() {
            ...
            public void run() {
                ...
                if (duration > 0) {
                    ...
                    if (elapsedTime % 60 == 0 && elapsedTime < duration && elapsedTime != 0) {
                        System.err.println("Minute Passed" + elapsedTime);
                        onMinutePassed();
                    }
                }
    ...
    protected abstract void onMinutePassed();

    /**
     * @return the elapsed time (in seconds) since the start of the timer.
     */
    ...
    /**
     * @return the time remaining (in seconds) for the timer to stop. If the
    ...
    public void addInElapsedTime(long time) {
        elapsedTime += time;
    }
}
